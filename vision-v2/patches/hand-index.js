...
async function loadHandDetectorModel(_HANDDETECT_MODEL_PATH, fromHub) {
    var HANDDETECT_MODEL_PATH = _HANDDETECT_MODEL_PATH || 'https://tfhub.dev/mediapipe/tfjs-model/handdetector/1/default/1';
    return tfconv.loadGraphModel(HANDDETECT_MODEL_PATH, { fromTFHub: fromHub || true });
}
const MESH_MODEL_INPUT_WIDTH = 256;
const MESH_MODEL_INPUT_HEIGHT = 256;
async function loadHandPoseModel(_HANDPOSE_MODEL_PATH, fromHub) {
    var HANDPOSE_MODEL_PATH = HANDPOSE_MODEL_PATH || 'https://tfhub.dev/mediapipe/tfjs-model/handskeleton/1/default/1';
    return tfconv.loadGraphModel(HANDPOSE_MODEL_PATH, { fromTFHub: fromHub || true });
}
async function loadAnchors(PATH) {
    return tf.util
        .fetch(PATH || 'https://tfhub.dev/mediapipe/tfjs-model/handskeleton/1/default/1/anchors.json?tfjs-format=file')
        .then(d => d.json());
}
async function load({ maxContinuousChecks = Infinity, detectionConfidence = 0.8, iouThreshold = 0.3, scoreThreshold = 0.5,
    APATH = 'https://tfhub.dev/mediapipe/tfjs-model/handskeleton/1/default/1/anchors.json?tfjs-format=file',
    _HANDPOSE_MODEL_PATH = 'https://tfhub.dev/mediapipe/tfjs-model/handskeleton/1/default/1',
    _HANDDETECT_MODEL_PATH = 'https://tfhub.dev/mediapipe/tfjs-model/handdetector/1/default/1',
    TFHUB = true
} = {}) {
    const [ANCHORS, handDetectorModel, handPoseModel] = await Promise.all([loadAnchors(APATH), loadHandDetectorModel(
        _HANDDETECT_MODEL_PATH, TFHUB
    ), loadHandPoseModel(
        _HANDPOSE_MODEL_PATH, TFHUB
    )]);
